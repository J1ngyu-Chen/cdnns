...
kernel_sigmoid_fwd_default_ow_f32(size_t batch_size, size_t inout_dim,
				  const float *__restrict input,
				  float *__restrict output);

static __global__ void
kernel_sigmoid_fwd_default_ow_in_place_f32(size_t batch_size, size_t inout_dim,
					   float *inout);

static __global__ void
kernel_sigmoid_fwd_default_accum_f32(size_t batch_size, size_t inout_dim,
				     const float *__restrict input,
				     float *__restrict output);

static __global__ void
kernel_sigmoid_fwd_default_ow_f64(size_t batch_size, size_t inout_dim,
				  const double *__restrict input,
				  double *__restrict output);

static __global__ void
kernel_sigmoid_fwd_default_ow_in_place_f64(size_t batch_size, size_t inout_dim,
					   double *inout);

static __global__ void
kernel_sigmoid_fwd_default_accum_f64(size_t batch_size, size_t inout_dim,
				     const double *__restrict input,
				     double *__restrict output);
				...
				float *__restrict output)
{
	...
	kernel_sigmoid_fwd_default_ow_f32<<<grid_dim, block_dim>>>(
					 ...
					 float *inout)
{
	...
	dim3 grid_dim((inout_dim - 1) / block_dim.x + 1,
		      (batch_size - 1) / block_dim.y + 1);
	kernel_sigmoid_fwd_default_ow_in_place_f32<<<grid_dim, block_dim>>>(
				   ...
				   float *__restrict output)
{
	...
	dim3 grid_dim((inout_dim - 1) / block_dim.x + 1,
		      (batch_size - 1) / block_dim.y + 1);
	kernel_sigmoid_fwd_default_accum_f32<<<grid_dim, block_dim>>>(
		batch_size, inout_dim, input, output);
}
				...
				double *__restrict output)
{
	...
	dim3 grid_dim((inout_dim - 1) / block_dim.x + 1,
		      (batch_size - 1) / block_dim.y + 1);
	kernel_sigmoid_fwd_default_ow_f64<<<grid_dim, block_dim>>>(
		batch_size, inout_dim, input, output);
}
					 ...
					 double *inout)
{
	...
	dim3 grid_dim((inout_dim - 1) / block_dim.x + 1,
		      (batch_size - 1) / block_dim.y + 1);
	kernel_sigmoid_fwd_default_ow_in_place_f64<<<grid_dim, block_dim>>>(
		batch_size, inout_dim, inout);
}
				   ...
				   double *__restrict output)
{
	...
	dim3 grid_dim((inout_dim - 1) / block_dim.x + 1,
		      (batch_size - 1) / block_dim.y + 1);
	kernel_sigmoid_fwd_default_accum_f64<<<grid_dim, block_dim>>>(
		batch_size, inout_dim, input, output);
}
}
