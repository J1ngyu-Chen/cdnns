		      ...
		      const void *weight, const void *bias, void *output)
{
	...
	case CDNNS_TYPE_FP32:
		...
		case CDNNS_MODE_OVERWRITE:
			...
			case CDNNS_OPTION_DEFAULT:
				linear_fwd_default_ow_f32(batch_size, in_dim,
							  out_dim, input,
							  weight, bias, output);
				break;
			case CDNNS_OPTION_NO_BIAS:
				linear_fwd_no_bias_ow_f32(batch_size, in_dim,
							  out_dim, input,
							  weight, output);
				break;
			case CDNNS_OPTION_FUSE_RELU:
				linear_fwd_fuse_relu_ow_f32(batch_size, in_dim,
							    out_dim, input,
							    weight, bias,
							    output);
				break;
		...
		case CDNNS_MODE_ACCUMULATE:
			...
			switch (option) {
			case CDNNS_OPTION_DEFAULT:
				linear_fwd_default_accum_f32(batch_size, in_dim,
							     out_dim, input,
							     weight, bias,
							     output);
				break;
			case CDNNS_OPTION_NO_BIAS:
				linear_fwd_no_bias_accum_f32(batch_size, in_dim,
							     out_dim, input,
							     weight, output);
				break;
			case CDNNS_OPTION_FUSE_RELU:
				linear_fwd_fuse_relu_accum_f32(batch_size,
							       in_dim, out_dim,
							       input, weight,
							       bias, output);
				break;
	...
	case CDNNS_TYPE_FP64:
		...
		case CDNNS_MODE_OVERWRITE:
			...
			switch (option) {
			case CDNNS_OPTION_DEFAULT:
				linear_fwd_default_ow_f64(batch_size, in_dim,
							  out_dim, input,
							  weight, bias, output);
				break;
			case CDNNS_OPTION_NO_BIAS:
				linear_fwd_no_bias_ow_f64(batch_size, in_dim,
							  out_dim, input,
							  weight, output);
				break;
			case CDNNS_OPTION_FUSE_RELU:
				linear_fwd_fuse_relu_ow_f64(batch_size, in_dim,
							    out_dim, input,
							    weight, bias,
							    output);
				break;
		...
		case CDNNS_MODE_ACCUMULATE:
			...
			switch (option) {
			case CDNNS_OPTION_DEFAULT:
				linear_fwd_default_accum_f64(batch_size, in_dim,
							     out_dim, input,
							     weight, bias,
							     output);
				break;
			case CDNNS_OPTION_NO_BIAS:
				linear_fwd_no_bias_accum_f64(batch_size, in_dim,
							     out_dim, input,
							     weight, output);
				break;
			case CDNNS_OPTION_FUSE_RELU:
				linear_fwd_fuse_relu_accum_f64(batch_size,
							       in_dim, out_dim,
							       input, weight,
							       bias, output);
				break;
...
void cdnns_linear_backward(enum cdnns_type type, enum cdnns_mode mode,
			   ...
			   void *__restrict input_grad,
			   const void *__restrict weight,
			   void *__restrict weight_grad,
			   void *__restrict bias_grad,
			   const void *__restrict output_grad)
{
...
void cdnns_linear_backward_f64(
	const double *__restrict input, double *__restrict input_grad,
	const double *__restrict weight, double *__restrict weight_grad,
	double *__restrict bias_grad,
	const double *__restrict output_grad,
	size_t batch_size,
